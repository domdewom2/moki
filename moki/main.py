...
Usage:
    python -m moki              # Windowed (development)
    python -m moki --fullscreen # Fullscreen (Pi)
    python -m moki --mock       # Mock mode (UI testing)
"""
import os
import sys
import logging
...
from .app import Moki


def setup_logging():
    """Configure logging with console and rotating file handler."""
    # Determine log level from environment or default to INFO
    level_name = os.environ.get('MOKI_LOG_LEVEL', 'INFO').upper()
...
def main():
    """Entry point for Moki application."""
    setup_logging()
    ...
    app = Moki(fullscreen=FULLSCREEN)
    app.start()
